package main

import (
	"bufio"
	"context"
	"encoding/json"
	"flag"
	"fmt"
	"io"
	"log"
	"net"
	"os"
	"strings"
	"sync"
	"sync/atomic"
	"time"

	"golang.org/x/time/rate"
)

// ============================================================
// Config
// ============================================================

type ProxyConfig struct {
	Name        string `json:"name"`
	Listen      int    `json:"listen"`
	Target      string `json:"target"`
	Up          string `json:"up"`
	Down        string `json:"down"`
	Burst       string `json:"burst,omitempty"`

	// Connection limits to protect the backend from multi-connection abuse
	// (IDM, download accelerators, etc.).
	MaxConns       int     `json:"max_conns,omitempty"`        // total concurrent conns for this proxy (0 = unlimited)
	MaxConnsPerIP  int     `json:"max_conns_per_ip,omitempty"` // concurrent conns per client IP (0 = unlimited)
	NewConnsPerSec float64 `json:"new_conns_per_sec,omitempty"` // rate of new conns accepted (0 = unlimited)
	IdleTimeoutSec int     `json:"idle_timeout_sec,omitempty"` // close conns with no traffic for this long (0 = disabled)
}

// loadConfigs reads a JSONL file (one JSON object per line).
func loadConfigs(path string) ([]ProxyConfig, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, fmt.Errorf("open config: %w", err)
	}
	defer f.Close()

	var configs []ProxyConfig
	scanner := bufio.NewScanner(f)
	scanner.Buffer(make([]byte, 1024*1024), 1024*1024)

	lineNum := 0
	for scanner.Scan() {
		lineNum++
		line := strings.TrimSpace(scanner.Text())
		if line == "" || strings.HasPrefix(line, "#") || strings.HasPrefix(line, "//") {
			continue
		}

		var cfg ProxyConfig
		if err := json.Unmarshal([]byte(line), &cfg); err != nil {
			return nil, fmt.Errorf("line %d: invalid JSON: %w", lineNum, err)
		}

		if err := validateConfig(&cfg, lineNum); err != nil {
			return nil, err
		}

		configs = append(configs, cfg)
	}

	if err := scanner.Err(); err != nil {
		return nil, fmt.Errorf("read config: %w", err)
	}

	if len(configs) == 0 {
		return nil, fmt.Errorf("no valid proxy entries found in %s", path)
	}

	return configs, nil
}

func validateConfig(cfg *ProxyConfig, lineNum int) error {
	if cfg.Name == "" {
		cfg.Name = fmt.Sprintf("proxy-%d", cfg.Listen)
	}
	if cfg.Listen <= 0 || cfg.Listen > 65535 {
		return fmt.Errorf("line %d (%s): invalid listen port %d", lineNum, cfg.Name, cfg.Listen)
	}
	if cfg.Target == "" {
		return fmt.Errorf("line %d (%s): target is required", lineNum, cfg.Name)
	}
	if cfg.Up == "" {
		return fmt.Errorf("line %d (%s): up is required", lineNum, cfg.Name)
	}
	if cfg.Down == "" {
		return fmt.Errorf("line %d (%s): down is required", lineNum, cfg.Name)
	}
	return nil
}

// ============================================================
// Stats
// ============================================================

type ProxyStats struct {
	Name          string
	UploadBytes   uint64
	DownloadBytes uint64
	Connections   int64
	TotalConns    uint64
	RejectedConns uint64 // conns dropped due to limits
}

var (
	allStats   = make(map[string]*ProxyStats)
	statsMutex sync.RWMutex
)

func getOrCreateStats(name string) *ProxyStats {
	statsMutex.Lock()
	defer statsMutex.Unlock()
	s, ok := allStats[name]
	if !ok {
		s = &ProxyStats{Name: name}
		allStats[name] = s
	}
	return s
}

// ============================================================
// Rate Limited Reader
// ============================================================

type rateLimitedReader struct {
	r       io.Reader
	limiter *rate.Limiter
	counter *uint64
	ctx     context.Context
	// lastActivity is updated on every successful read, used for idle detection.
	lastActivity *int64
}

func (rlr *rateLimitedReader) Read(p []byte) (int, error) {
	const maxChunk = 16 * 1024
	if len(p) > maxChunk {
		p = p[:maxChunk]
	}

	n, err := rlr.r.Read(p)
	if n > 0 {
		atomic.AddUint64(rlr.counter, uint64(n))
		if rlr.lastActivity != nil {
			atomic.StoreInt64(rlr.lastActivity, time.Now().UnixNano())
		}
		if waitErr := rlr.limiter.WaitN(rlr.ctx, n); waitErr != nil {
			return n, waitErr
		}
	}
	return n, err
}

// ============================================================
// Per-IP connection tracker
// ============================================================

type ipTracker struct {
	mu     sync.Mutex
	counts map[string]int
}

func newIPTracker() *ipTracker {
	return &ipTracker{counts: make(map[string]int)}
}

// tryAcquire returns true if this IP can open another connection (under limit).
func (t *ipTracker) tryAcquire(ip string, limit int) bool {
	if limit <= 0 {
		return true // unlimited
	}
	t.mu.Lock()
	defer t.mu.Unlock()
	if t.counts[ip] >= limit {
		return false
	}
	t.counts[ip]++
	return true
}

func (t *ipTracker) release(ip string) {
	t.mu.Lock()
	defer t.mu.Unlock()
	if t.counts[ip] > 0 {
		t.counts[ip]--
		if t.counts[ip] == 0 {
			delete(t.counts, ip)
		}
	}
}

// ============================================================
// Proxy
// ============================================================

type Proxy struct {
	name         string
	listenPort   int
	targetAddr   string
	uploadRate   int
	downloadRate int
	burst        int
	stats        *ProxyStats

	uploadLimiter   *rate.Limiter
	downloadLimiter *rate.Limiter

	// Connection protection
	maxConns       int64           // 0 = unlimited
	maxConnsPerIP  int             // 0 = unlimited
	newConnLimiter *rate.Limiter   // nil = unlimited
	idleTimeout    time.Duration   // 0 = disabled
	ipTracker      *ipTracker

	// connSem is a simple semaphore for total concurrent connections.
	connSem chan struct{}
}

func NewProxy(cfg ProxyConfig) (*Proxy, error) {
	uploadRate, err := parseRate(cfg.Up)
	if err != nil {
		return nil, fmt.Errorf("%s: upload rate: %w", cfg.Name, err)
	}
	downloadRate, err := parseRate(cfg.Down)
	if err != nil {
		return nil, fmt.Errorf("%s: download rate: %w", cfg.Name, err)
	}

	// Default burst: large enough for 16KB chunks + headroom, small enough
	// to keep the rate honest.
	var burst int
	if cfg.Burst == "" {
		burst = downloadRate
		if uploadRate > burst {
			burst = uploadRate
		}
		if burst < 64*1024 {
			burst = 64 * 1024
		}
		if burst > 256*1024 {
			burst = 256 * 1024
		}
	} else {
		burst, err = parseRate(cfg.Burst)
		if err != nil {
			return nil, fmt.Errorf("%s: burst: %w", cfg.Name, err)
		}
		if burst < 16*1024 {
			burst = 16 * 1024
		}
	}

	p := &Proxy{
		name:            cfg.Name,
		listenPort:      cfg.Listen,
		targetAddr:      cfg.Target,
		uploadRate:      uploadRate,
		downloadRate:    downloadRate,
		burst:           burst,
		stats:           getOrCreateStats(cfg.Name),
		uploadLimiter:   rate.NewLimiter(rate.Limit(uploadRate), burst),
		downloadLimiter: rate.NewLimiter(rate.Limit(downloadRate), burst),
		maxConns:        int64(cfg.MaxConns),
		maxConnsPerIP:   cfg.MaxConnsPerIP,
		idleTimeout:     time.Duration(cfg.IdleTimeoutSec) * time.Second,
		ipTracker:       newIPTracker(),
	}

	if cfg.MaxConns > 0 {
		p.connSem = make(chan struct{}, cfg.MaxConns)
	}
	if cfg.NewConnsPerSec > 0 {
		// burst = at most 2 seconds worth of new connections
		b := int(cfg.NewConnsPerSec * 2)
		if b < 1 {
			b = 1
		}
		p.newConnLimiter = rate.NewLimiter(rate.Limit(cfg.NewConnsPerSec), b)
	}

	return p, nil
}

func (p *Proxy) Start(ctx context.Context) error {
	listener, err := net.Listen("tcp", fmt.Sprintf(":%d", p.listenPort))
	if err != nil {
		return fmt.Errorf("[%s] listen :%d: %w", p.name, p.listenPort, err)
	}
	defer listener.Close()

	log.Printf("[%s] listening on :%d -> %s", p.name, p.listenPort, p.targetAddr)
	log.Printf("[%s]   rates: up %s/s, down %s/s, burst %s",
		p.name, humanizeBytes(p.uploadRate), humanizeBytes(p.downloadRate), humanizeBytes(p.burst))
	log.Printf("[%s]   limits: max_conns=%d, per_ip=%d, new_per_sec=%.1f, idle_timeout=%s",
		p.name, p.maxConns, p.maxConnsPerIP,
		rateLimitRate(p.newConnLimiter), p.idleTimeout)

	go func() {
		<-ctx.Done()
		listener.Close()
	}()

	for {
		clientConn, err := listener.Accept()
		if err != nil {
			if ctx.Err() != nil {
				return nil
			}
			log.Printf("[%s] accept error: %v", p.name, err)
			continue
		}

		// Apply admission control BEFORE spawning a goroutine - cheap and fast.
		if !p.admit(clientConn) {
			atomic.AddUint64(&p.stats.RejectedConns, 1)
			clientConn.Close()
			continue
		}

		go p.handleConnection(clientConn)
	}
}

// admit decides whether to accept this connection based on configured limits.
// Returns false if the connection should be rejected immediately.
func (p *Proxy) admit(conn net.Conn) bool {
	// 1. New-connection rate limit: use non-blocking Allow() so we drop bursts
	//    instead of queueing them (queueing would still hit the backend hard).
	if p.newConnLimiter != nil && !p.newConnLimiter.Allow() {
		return false
	}

	// 2. Total concurrent connections: non-blocking acquire.
	if p.connSem != nil {
		select {
		case p.connSem <- struct{}{}:
		default:
			return false
		}
	}

	// 3. Per-IP concurrent connections.
	ip := clientIP(conn)
	if !p.ipTracker.tryAcquire(ip, p.maxConnsPerIP) {
		if p.connSem != nil {
			<-p.connSem // release the slot we just took
		}
		return false
	}

	return true
}

func (p *Proxy) handleConnection(clientConn net.Conn) {
	ip := clientIP(clientConn)

	defer func() {
		clientConn.Close()
		p.ipTracker.release(ip)
		if p.connSem != nil {
			<-p.connSem
		}
	}()

	atomic.AddInt64(&p.stats.Connections, 1)
	atomic.AddUint64(&p.stats.TotalConns, 1)
	defer atomic.AddInt64(&p.stats.Connections, -1)

	targetConn, err := net.DialTimeout("tcp", p.targetAddr, 10*time.Second)
	if err != nil {
		log.Printf("[%s] dial %s: %v", p.name, p.targetAddr, err)
		return
	}
	defer targetConn.Close()

	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	// Track last activity across both directions for the idle watchdog.
	lastActivity := time.Now().UnixNano()

	// Idle watchdog: closes both ends if no data flows for idleTimeout.
	if p.idleTimeout > 0 {
		go p.idleWatchdog(ctx, &lastActivity, clientConn, targetConn)
	}

	var wg sync.WaitGroup
	wg.Add(2)

	// client -> server (upload)
	go func() {
		defer wg.Done()
		defer func() {
			if tc, ok := targetConn.(*net.TCPConn); ok {
				tc.CloseWrite()
			}
		}()
		limited := &rateLimitedReader{
			r:            clientConn,
			limiter:      p.uploadLimiter,
			counter:      &p.stats.UploadBytes,
			ctx:          ctx,
			lastActivity: &lastActivity,
		}
		io.Copy(targetConn, limited)
	}()

	// server -> client (download)
	go func() {
		defer wg.Done()
		defer func() {
			if tc, ok := clientConn.(*net.TCPConn); ok {
				tc.CloseWrite()
			}
		}()
		limited := &rateLimitedReader{
			r:            targetConn,
			limiter:      p.downloadLimiter,
			counter:      &p.stats.DownloadBytes,
			ctx:          ctx,
			lastActivity: &lastActivity,
		}
		io.Copy(clientConn, limited)
	}()

	wg.Wait()
}

func (p *Proxy) idleWatchdog(ctx context.Context, lastActivity *int64, conns ...net.Conn) {
	check := p.idleTimeout / 2
	if check < time.Second {
		check = time.Second
	}
	ticker := time.NewTicker(check)
	defer ticker.Stop()

	for {
		select {
		case <-ctx.Done():
			return
		case now := <-ticker.C:
			last := atomic.LoadInt64(lastActivity)
			if now.UnixNano()-last > int64(p.idleTimeout) {
				for _, c := range conns {
					_ = c.Close()
				}
				return
			}
		}
	}
}

// ============================================================
// Helpers
// ============================================================

func clientIP(c net.Conn) string {
	addr := c.RemoteAddr().String()
	host, _, err := net.SplitHostPort(addr)
	if err != nil {
		return addr
	}
	return host
}

func rateLimitRate(l *rate.Limiter) float64 {
	if l == nil {
		return 0
	}
	return float64(l.Limit())
}

// ============================================================
// Stats printer
// ============================================================

func printStats(interval time.Duration) {
	ticker := time.NewTicker(interval)
	defer ticker.Stop()

	type snapshot struct {
		upload, download uint64
	}
	last := make(map[string]snapshot)
	lastTime := time.Now()

	for range ticker.C {
		now := time.Now()
		elapsed := now.Sub(lastTime).Seconds()
		lastTime = now

		statsMutex.RLock()
		names := make([]string, 0, len(allStats))
		for name := range allStats {
			names = append(names, name)
		}
		statsMutex.RUnlock()

		log.Printf("------ stats ------")
		for _, name := range names {
			statsMutex.RLock()
			s := allStats[name]
			statsMutex.RUnlock()

			up := atomic.LoadUint64(&s.UploadBytes)
			down := atomic.LoadUint64(&s.DownloadBytes)
			conns := atomic.LoadInt64(&s.Connections)
			total := atomic.LoadUint64(&s.TotalConns)
			rejected := atomic.LoadUint64(&s.RejectedConns)

			prev := last[name]
			upRate := float64(up-prev.upload) / elapsed
			downRate := float64(down-prev.download) / elapsed

			log.Printf("[%s] conns: %d/%d (rejected: %d) | up: %s/s | down: %s/s | total: up=%s down=%s",
				name, conns, total, rejected,
				humanizeBytes(int(upRate)),
				humanizeBytes(int(downRate)),
				humanizeBytes(int(up)),
				humanizeBytes(int(down)),
			)

			last[name] = snapshot{upload: up, download: down}
		}
	}
}

func humanizeBytes(b int) string {
	const unit = 1024
	if b < unit {
		return fmt.Sprintf("%d B", b)
	}
	div, exp := int64(unit), 0
	for n := b / unit; n >= unit; n /= unit {
		div *= unit
		exp++
	}
	return fmt.Sprintf("%.2f %cB", float64(b)/float64(div), "KMGTPE"[exp])
}

func parseRate(s string) (int, error) {
	s = strings.TrimSpace(strings.ToUpper(s))
	multiplier := 1
	switch {
	case strings.HasSuffix(s, "KB"):
		multiplier = 1024
		s = strings.TrimSuffix(s, "KB")
	case strings.HasSuffix(s, "MB"):
		multiplier = 1024 * 1024
		s = strings.TrimSuffix(s, "MB")
	case strings.HasSuffix(s, "GB"):
		multiplier = 1024 * 1024 * 1024
		s = strings.TrimSuffix(s, "GB")
	case strings.HasSuffix(s, "B"):
		s = strings.TrimSuffix(s, "B")
	}

	var value int
	if _, err := fmt.Sscanf(strings.TrimSpace(s), "%d", &value); err != nil {
		return 0, fmt.Errorf("invalid rate format: %q (example: 30KB, 1MB)", s)
	}
	return value * multiplier, nil
}

// ============================================================
// Main
// ============================================================

func main() {
	var (
		configPath    = flag.String("config", "config.json", "path to JSONL config file")
		statsInterval = flag.Duration("stats-interval", 5*time.Second, "stats print interval")
	)
	flag.Parse()

	configs, err := loadConfigs(*configPath)
	if err != nil {
		log.Fatalf("load config: %v", err)
	}

	log.Printf("loaded %d proxy entries from %s", len(configs), *configPath)

	seen := make(map[int]string)
	for _, c := range configs {
		if prev, ok := seen[c.Listen]; ok {
			log.Fatalf("duplicate listen port %d used by %q and %q", c.Listen, prev, c.Name)
		}
		seen[c.Listen] = c.Name
	}

	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	var wg sync.WaitGroup
	for _, cfg := range configs {
		p, err := NewProxy(cfg)
		if err != nil {
			log.Fatalf("create proxy: %v", err)
		}

		wg.Add(1)
		go func(proxy *Proxy) {
			defer wg.Done()
			if err := proxy.Start(ctx); err != nil {
				log.Printf("[%s] stopped: %v", proxy.name, err)
			}
		}(p)
	}

	go printStats(*statsInterval)

	wg.Wait()
}
